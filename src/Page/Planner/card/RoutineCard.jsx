...
const RoutineCard = (p) => {
  ...
  const AllDZ = () => (
    <Fragment>
      ...
      <DateZoneLabel
        name="must-do"
        className="mb-10"
        title="Không cần phải làm"
        num={dateType?.doNotNeed?.length ?? 0}
      />
      <TaskCardList className="mb-30">
  ...
  return (
    <Container>{dateZone === "today" ? <TodayDZ /> : <AllDZ />}</Container>
  );
};
                  ...
                  .map((date) => {
                    if (date.check)
                      return (
                        <span
                          key={date.id}
                          onClick={() => handleClickUnCheckDate(date.value)}
                        >
                          <Img.routineDone />
                        </span>
                      );
                      ...
                      return (
                        <span
                          key={date.id}
                          onClick={() => handleClickCheckDate(date.value)}
                        >
                          <Img.routineMiss />
                        </span>
                      );
...
const Option = (p) => {
  ...
  const { handleDeleteRoutine, handleUpdateRoutine } =
    useContext(RoutineContext);
