...
const FaceSVG = (p) => {
  ...
  useEffect(() => {
    ...
    const armL = svgContainer.querySelector('.armL');
    const armR = svgContainer.querySelector('.armR');
    const twoFingers = svgContainer.querySelector('.twoFingers');
    const eyeL = svgContainer.querySelector('.eyeL');
    const eyeR = svgContainer.querySelector('.eyeR');
    const nose = svgContainer.querySelector('.nose');
    const mouth = svgContainer.querySelector('.mouth');
    const mouthBG = svgContainer.querySelector('.mouthBG');
    const mouthSmallBG = svgContainer.querySelector('.mouthSmallBG');
    const mouthMediumBG = svgContainer.querySelector('.mouthMediumBG');
    const mouthLargeBG = svgContainer.querySelector('.mouthLargeBG');
    const mouthOutline = svgContainer.querySelector('.mouthOutline');
    const mouthMaskPath = svgContainer.querySelector('#mouthMaskPath');
    const tooth = svgContainer.querySelector('.tooth');
    const tongue = svgContainer.querySelector('.tongue');
    const chin = svgContainer.querySelector('.chin');
    const face = svgContainer.querySelector('.face');
    const eyebrow = svgContainer.querySelector('.eyebrow');
    const outerEarL = svgContainer.querySelector('.outerEarL');
    const outerEarR = svgContainer.querySelector('.outerEarR');
    const earHairL = svgContainer.querySelector('.earHairL');
    const earHairR = svgContainer.querySelector('.earHairR');
    const hair = svgContainer.querySelector('.hair');
    const bodyBG = svgContainer.querySelector('.bodyBGnormal');
    const bodyBGchanged = svgContainer.querySelector('.bodyBGchanged');

    let activeElement, screenCenter, emailCoords, svgCoords;
    ...
    const coverEyes = () => {
      ...
      gsap.to(armL, { x: -93, y: 10, rotation: 0, ease: 'quad.out' });
      gsap.to(armR, { x: -93, y: 10, rotation: 0, ease: 'quad.out', delay: 0.1 });
      gsap.to(bodyBG, { morphSVG: bodyBGchanged, ease: 'quad.out' });
      eyesCovered = true;
    };

    const uncoverEyes = () => {
      gsap.killTweensOf([armL, armR]);
      gsap.to(armL, { y: 220, ease: 'quad.out' });
      gsap.to(armL, { rotation: 105, ease: 'quad.out', delay: 0.1 });
      gsap.to(armR, { y: 220, ease: 'quad.out' });
      gsap.to(armR, {
        rotation: -105,
        ease: 'quad.out',
        delay: 0.1,
      ...
      gsap.to(bodyBG, { morphSVG: bodyBG, ease: 'quad.out' });
      eyesCovered = false;
    };

  
    const spreadFingers = () => {
      console.log("spread fingers");
      gsap.to(twoFingers, {
    ...
    const initLoginForm = () => {
      svgCoords = svgContainer.getBoundingClientRect();
      emailCoords = email.getBoundingClientRect();
      screenCenter = svgCoords.left + svgContainer.offsetWidth / 2;

      email.addEventListener('focus', onEmailFocus);
      email.addEventListener('blur', onEmailBlur);
      email.addEventListener('input', onEmailInput);
      checkBox.addEventListener('change', handleChange);
      checkBox.addEventListener('blur', onPasswordBlur);
  ...
  //     if (!twoFingers) return;  // Ensure the element exists
  ...
  >
    ...
    <g className="earL">
      <g className="outerEar" fill="#ddf1fa" stroke="#6a6d70" strokeWidth="2.5">
    ...
    <g className="eyeL">
      <circle cx="85.5" cy="78.5" r="3.5" fill="#6e6e6e" />
      <circle cx="84" cy="76" r="1" fill="#fff" />
    </g>
    <g className="eyeR">
      <circle cx="114.5" cy="78.5" r="3.5" fill="#747474" />
